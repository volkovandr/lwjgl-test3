...
import org.lwjgl.opengl.GL11.glGetTexLevelParameteriv
...
class Texture {
    ...
    def loadFromResource(resourceName: String): Unit = {
        ...
        Using(MemoryStack.stackPush()) { stack =>
            val pWidth = stack.ints(1)
            val pHeight = stack.ints(1)
            val mipLevel = 0
            glGetTexLevelParameteriv(GL_TEXTURE_2D, mipLevel, GL_TEXTURE_WIDTH, pWidth)
            glGetTexLevelParameteriv(GL_TEXTURE_2D, mipLevel, GL_TEXTURE_WIDTH, pHeight)
            println(s"Loaded texture from [$resourceName] ${pWidth.get()}x${pHeight.get()} ID=$textureId")
        }
    }
